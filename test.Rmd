...
`r max.message("发送")`
```
使用最多的默认表情为 `r paste(sendEmoji[1]$Emoji, sendEmoji[1]$N)`次和 `r paste(sendEmoji[2]$Emoji, sendEmoji[2]$N)` 次  
...
`r max.message("接收")`
```
使用最多的默认表情为 `r paste(receivedEmoji[1]$Emoji, receivedEmoji[1]$N)`次和 `r paste(receivedEmoji[2]$Emoji, receivedEmoji[2]$N)` 次  